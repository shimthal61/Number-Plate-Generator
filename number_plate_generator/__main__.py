...
def main() -> None:
    ...
    if len(sys.argv) != 3:
        ...
        print("       python -m number_plate_generator --reset")
        print("Example: python -m number_plate_generator MV 03/04/2010")
        sys.exit(1)

    memory_tag = sys.argv[1].upper()
    date = sys.argv[2]

    # Uses the default state file so history persists across runs automatically.
